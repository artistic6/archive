...
for ($raceNumber = 1; $raceNumber <= $numberOfRaces; $raceNumber++) {
    ...
    $racetext .= "\t\t'bets' => [\n";
    if(!empty($toWin)){
        $racetext .= "\t\t\t'win(count $raceDate, $" . $unitBet . ")' => '" . implode(", ", $toWin) . "',\n"; 
    ...
    if(!empty($allValues) && !in_array($raceNumber, [3, 4, 5, 6, 8]) && count($allValues) <= 7){
        ...
        $racetext .= "\t\t\t'win(allValues $raceDate, $" . $unitBet . ")' => '" . implode(", ", $allValues) . "',\n"; 
    ...
    if(count($favorites) >= 3 && count($winInter) >= 3 && !in_array(end($favorites), $diff)){
        $racetext .= "\t\t\t'place(end-favorites $raceDate, $" . 2 * $unitBet . ")' => '" .  end($favorites)  . "',\n"; 
    ...
    if(in_array(count($winSets), $allValuesWin) && in_array(count(array_intersect($allValues, $favorites)), [2, 3])){
        ...
        if(count($wp) < 9){
            $racetext .= "\t\t\t'win(wp $raceDate, " . $unitBet . ")' => '" . implode(", ", $wp) . "',\n"; 
        ...
        if(count($wp) === 3){
            $racetext .= "\t\t\t'place(end-wp $raceDate, $" . 2 * $unitBet . ")' => '" . end($wp) . "',\n"; 
            $totalBets[$raceNumber] += 2 * $unitBet;
            $totalPlace -= 2 * $unitBet;
